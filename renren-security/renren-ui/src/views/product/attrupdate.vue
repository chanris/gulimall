...
export default {
	...
	methods: {
		...
		getSpuBaseAttrs() {
			...
			//   });
			let _that = this
			productService.get(`/product/attr/base/listforspu/${this.spuId}`)
				.then(({ data }) => {
					data.forEach(item => {
						this.spuAttrsMap["" + item.attrId] = item;
					});
		...
		getQueryParams() {
			...
			this.spuId = route.query.spuId;
			this.catalogId = route.query.catalogId;
		},
		showBaseAttrs() {
			let _this = this;
			//   this.$http({
			...
			productService.attrGroupListWithAttr(this.catalogId)
				.then(({ data }) => {
					//先对表单的baseAttrs进行初始化
					// console.log(data)
					data.forEach(item => {
		...
		submitSpuAttrs() {
			...
			.then(() => {
				productService.post(`/product/attr/update/${this.spuId}`, submitData)
				.then(({data})=>{
					ElMessage({
						type: 'success',
						message: `属性修改成功!`,
